...
//        uint32_t *bitmap;
    ...
    public:
        uint32_t *memory;
        nvram(int start_sector, int num_bytes, int flag) : flag(flag) {
            start = start_sector;
            num_words = Align(num_bytes, 2) / 4;
            int num_sectors = Align(num_words, 8) / 256;
            memory = new uint32_t[num_words];
//            bitmap = new uint32_t[(num_sectors + 31) >> 5];
        }
        int read(int verify_flag) {
            int offset = ((start & 0b11) << 2) | ((start & 0b1111100) << 10);
            return sector::read(memory, 0x08000000 + offset, num_words, (flag & INFO) | (verify_flag & VERIFY));
        }
        void write() {
            int num_sectors = Align(num_words, 8) / 256;
            int remain = num_words;
            for(int i = 0; i < num_sectors; i++) {
                int len = (remain < (1024 / 4)) ? remain : (1024 / 4);
                int offset = (((i + start) & 0b11) << 2) | (((i + start) & 0b1111100) << 10);
                int addr = 0x08000000 + offset;
                int r = sector::read(memory + i * 1024 / 4, addr, len, (flag & INFO) | VERIFY);
                if(r) {
                    erase_page_sector(addr, (flag & INFO));
                    sector::write(memory + i * 1024 / 4, addr, len, (flag & INFO));
                }
                remain -= len;
            }
        }
